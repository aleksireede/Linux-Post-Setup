...
def flatpak():
    subprocess.run(["flatpak", "remote-add", "--if-not-exists", "flathub", "https://flathub.org/repo/flathub.flatpakrepo"],check=True,text=True)
...
def debian_packages_install():
    ...
    download_file_from_url("/etc/apt/trusted.gpg.d/microsoft.asc","https://packages.microsoft.com/keys/microsoft.asc")
...
def download_file_from_url(output_file, content_url):
    if pathlib2.Path(output_file).exists():return
    subprocess.run(["sudo","touch",output_file],text=True,check=True)
    subprocess.run(["sudo","chmod","777",output_file],text=True,check=True)
